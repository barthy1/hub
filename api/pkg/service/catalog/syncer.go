...
func (s *syncer) updateResourcePlatforms(
	...
	platformIds := []uint{}
	for verPlatformId, _ := range verPlatformIds {
	...
	txn.Unscoped().Where(&model.ResourcePlatform{ResourceID: res.ID}).Not(map[string]interface{}{"platform_id": platformIds}).Delete(&model.ResourcePlatform{})
}
...
func (s *syncer) updateResourceVersions(
	...
	for _, v := range versions {
		...
		// remove mapping for the platforms, which are not in the list
		txn.Unscoped().Where(&model.VersionPlatform{ResourceVersionID: ver.ID}).Not(map[string]interface{}{"platform_id": platformIds}).Delete(&model.VersionPlatform{})
	}
}
